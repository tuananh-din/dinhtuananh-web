...
<section class="about-counter-section fix section-padding pt-0">
    <div class="container">
        <div class="row g-4 align-items-stretch about-balanced-row">
            <div class="col-lg-7 d-flex">
                <div class="about-grid-card about-grid-card--balanced w-100">
                    <h2>{{ $about->name }}</h2>
                    <p>{{ $about->description }}</p>
                    {!! $about->about_me !!}
                </div>
            </div>
            <div class="col-lg-5 d-flex" id="contact-brand">
                <div class="about-grid-card about-grid-card--balanced w-100">
        ...
        <div class="row g-4 align-items-stretch about-balanced-row">
            ...
            <div class="col-lg-6 d-flex">
                <div class="about-grid-card about-grid-card--balanced w-100">
                    ...
                    <div class="mb-4 about-skill-progress" data-progress="{{ $row->number }}">
                        <div class="d-flex justify-content-between">
                            <strong>{{ $row->name }}</strong>
                            <span>{{ $row->number }}%</span>
                        </div>
                        <p>{{ $row->description }}</p>
                        <div class="about-skill-progress__track" role="progressbar" aria-label="{{ $row->name }}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="{{ $row->number }}">
                            <span class="about-skill-progress__bar" style="--progress: {{ $row->number }}%;"></span>
                        </div>
                    </div>
                    @endforeach
                </div>
