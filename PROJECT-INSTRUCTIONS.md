...
- `tests/Feature/LoginLogoutTest.php`: đăng nhập đúng/sai và logout admin.
- `tests/Feature/PublicLeadSubmissionTest.php`: lead hợp lệ/honeypot/validation và lỗi email notification không làm mất lead.
- Chưa cover remember-me, hành vi guest của route login, CRUD admin khác, upload, slug, sitemap, SEO/JSON-LD, phân quyền, hay giao diện responsive.
