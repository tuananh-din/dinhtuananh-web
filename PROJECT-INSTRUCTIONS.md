...
- Ba form lead public (home, contact, course detail) dùng `autocomplete` cho name/tel/email; phone dùng `type="tel"` và `inputmode="tel"` để hỗ trợ bàn phím mobile. Giữ `name` field và validation server-side hiện có khi sửa form.
